    ...
    store.model.on('hit', (hitAreas) => {
        ...
        if (!playAudioing) {
            if (hitAreas.includes('摸头')) {
                ...
                playAudio(path.join(__dirname, '../../../model/hui/摸头.wav'))
            }

            if (hitAreas[0] === '摸胸' || hitAreas[1] === '摸手臂' || hitAreas[2] === '摸腿') {
                store.model.motion('tap_body')
                playAudio(path.join(__dirname, '../../../model/hui/摸胸.wav'))
            }

            if (hitAreas[0] === '摸手臂') {
                store.model.motion('tap_hand')
                playAudio(path.join(__dirname, '../../../model/hui/摸手.wav'))
            }

            if (hitAreas[0] === '摸腿') {
                store.model.motion('tap_shank')
                playAudio(path.join(__dirname, '../../../model/hui/摸腿.wav'))
            }
        }
    })
